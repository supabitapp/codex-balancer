...
func TestDashboardWebSocketStreamsEscapedHTML(t *testing.T) {
	stats := newStats()
	stats.routed("019fe5c2private", "a1b2c3d4", "unused", "gpt-5.6-sol", "high", serviceTierFast, transportWebSocket)
...
func TestDashboardRoutingShowsTokenUsage(t *testing.T) {
	now := time.Now()
	stats := newStats()
	stats.applyRouted(now, "thread", "client", "account", "gpt-5.6-sol", "xhigh", "", transportHTTP)
	usage := responseUsage{InputTokens: 2_000, OutputTokens: 300}
	usage.InputDetails.CachedTokens = 1_500
	...
	if thread.Model != "gpt-5.6-sol" || thread.Thinking != "xhigh" || thread.Input != "2K" || thread.Cached != "1.5K" || thread.Output != "300" {
	...
	body := string(payload)
	for _, expected := range []string{"<th>Model</th>", "<th>Thinking</th>", "<th>Input</th>", "<th>Cached</th>", "<th>Output</th>"} {
		if !strings.Contains(body, expected) {
			t.Fatalf("dashboard missing %q", expected)
		}
	}
