...
func connectAccount(tokens tokenResponse) (*Account, error) {
	if tokens.AccessToken == "" || claimsFromToken(tokens.IDToken).Auth.AccountID == "" {
		return nil, errors.New("account credentials are incomplete")
	}
